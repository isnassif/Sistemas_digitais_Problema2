...
<p>
  Para que se tornasse possível a comunicação entre o FPGA e o computador, foi necessário utilizar o protocolo HPS (Hard Processor System), responsável por passar as informações do computador para a placa utilizada. Essa troca de informações entre o HPS e a FPGA ocorre através de registradores mapeados em memória, permitindo que o software no HPS envie instruções e receba sinais de status do hardware. O protocolo segue uma sequência estruturada de três etapas principais:
   <strong>1. Enviar comando de instrução (<i>mapearPonte</i>)</strong>
<br>O HPS escreve o código da operação desejada (opcode) no registrador de controle (CONTROL_PIO_ptr), determinando qual algoritmo de processamento deve ser executado pela FPGA.</br>
